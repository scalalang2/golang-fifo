...
// This implementation is a copy of bigcache/queue/byte_queue.g
// ref. https://github.com/allegro/bigcache/blob/main/queue/bytes_queue.go
...
func (q *BytesQueue) canInsertAfterTail(need int) bool {
	...
	return q.head-q.tail == need || q.head-q.tail >= need
}
...
func (q *BytesQueue) canInsertBeforeHead(need int) bool {
	...
	if q.tail >= q.head {
		return q.head-leftMarginIndex == need || q.head-leftMarginIndex >= need
	}
	return q.head-q.tail == need || q.head-q.tail >= need
}
